      ...
      <SectionMain>
        ...
        <CardBox>
          ...
          <div className={'mb-4'}>
            ...
            <p>{prescriptions?.doctor?.firstName ?? 'No data'}</p>
          </div>

          <FormField label='IssuedDate'>
            {prescriptions.issued_date ? (
              <DatePicker
                ...
                selected={
                  prescriptions.issued_date
                    ? new Date(
                        dayjs(prescriptions.issued_date).format(
                          'YYYY-MM-DD hh:mm',
                        ),
                      )
                    : null
          ...
          <FormField label='Multi Text' hasTextareaHeight>
            <textarea
              className={'w-full'}
              disabled
              value={prescriptions?.medication_details}
            />
          </FormField>

          <div className={'mb-4'}>
            <p className={'block font-bold mb-2'}>Status</p>
            <p>{prescriptions?.status ?? 'No data'}</p>
          </div>
          ...
          {hasPermission(currentUser, 'READ_ORGANIZATIONS') && (
            <div className={'mb-4'}>
              ...
              <p>{prescriptions?.organization?.name ?? 'No data'}</p>
            </div>
          )}
